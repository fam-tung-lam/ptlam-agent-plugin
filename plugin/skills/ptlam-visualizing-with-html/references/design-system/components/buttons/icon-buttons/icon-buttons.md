...
standard variant for low emphasis, filled for the highest emphasis, tonal for a
quiet container, and outlined when the action needs a visible boundary.
...
must also expose `aria-pressed` or `aria-expanded`. A toggle may pair an
outlined resting icon with a filled selected icon. Keep narrow, default, and
wide visual widths inside a target of at least 44 pixels.
