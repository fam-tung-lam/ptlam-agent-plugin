...
Use `async` and `await` when the call path waits on I/O. An `async` label makes
nothing non-blocking: a synchronous file read, a synchronous crypto call, or a
long computation still occupies the single thread that runs every other task.
Move CPU-bound work to `worker_threads` or the project's job boundary.
...
supervisor with an explicit failure handler and shutdown path. Never start
untracked background work inside a request handler or a library call.

- Use `Promise.all` when one failure should abandon the group, and remember the
  siblings keep running; release their resources yourself.
...
Pass an `AbortSignal` down to the call that performs the I/O. Racing a promise
against a timer rejects the wrapper while the underlying request keeps running,
holding its socket and its memory:

```ts
const response = await fetch(url, { signal: AbortSignal.timeout(5_000) });
```

Tell a timeout apart from a caller's cancellation: one is the dependency
failing, the other is the caller no longer wanting the result. Let cancellation
travel on after cleanup instead of turning it into a success or a retry.
...
tasks unless its contract says that is safe.
