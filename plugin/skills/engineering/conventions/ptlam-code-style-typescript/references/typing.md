...
Type mechanics for TypeScript code. The project's `tsconfig.json` and installed
TypeScript version decide which syntax and checks are available;
...
substantially change. Let inference own local variables and obvious return
types; annotate a return when the function is exported or when inference would
widen the contract.

- Prefer a precise domain type over `any`. Use `unknown` for a really unknown
...
- Accept the widest useful shape and return the narrowest one: take
  `Iterable<T>` or `readonly T[]`, return the concrete type the caller gets.
- Mark data that must not change as `readonly`, on properties and on array and
  tuple parameters.
...
- Constrain every generic parameter, and remove a parameter that appears only
  once in the signature. It documents nothing.
...
instant, and a plain `YYYY-MM-DD` string for a calendar day with no instant.
...
the underlying error is gone. Keep it on the smallest expression and say why the
checker cannot prove the fact.
