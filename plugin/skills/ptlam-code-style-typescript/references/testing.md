...
existing runner when it is not Vitest.
...
non-deterministic external boundary.

Install the coverage package matching the installed Vitest version. Set
`coverage.include` to the production files in scope, so a file no test executes
still appears in the report. Apply a threshold only when the user or the
repository defines one; never invent a percentage and never lower one to make a
run pass. Coverage is evidence about executed lines, not about asserted
behavior.
...
resolves after the test has finished, so it passes whatever happens:
...
`beforeEach` that holds its real consumers. Register cleanup right after each
successful acquisition with `onTestFinished` or an `afterEach`, so a later setup
failure cannot leak the resource acquired earlier.
...
Restore anything global a test changes: `vi.useRealTimers` after
...
Use `toMatchSnapshot` when the whole stable structure is the behavior. For
...
and enable Vitest's `typecheck` option so those files run.

## Run and report

Use a file, name, or changed-file filter for a fast focused run. Use
`vitest run`, or the repository's script, for reproducible proof and in CI;
watch mode is an interactive loop, never final proof.

Run the repository's type check beside the suite. A passing suite says nothing
about type errors when the pipeline strips types without checking them.
