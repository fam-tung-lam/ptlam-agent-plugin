...
This reference owns the browser conditions an artifact must survive after static
...
| Offline reload            | The artifact opens with zero external network requests  |

Exercise every interactive step and every semantic-zoom level, not just the
default state. Reload with the browser offline and inspect its network log;
external requests fail this contract even when the static validator missed the
loading mechanism.
