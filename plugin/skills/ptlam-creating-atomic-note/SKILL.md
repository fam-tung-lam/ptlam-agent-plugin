...
Build durable notes from an idea, source, or existing note, with one useful
knowledge block each. This skill owns meaning, maturation, and connections. The
user's knowledge system owns storage, metadata, and links.

<!-- PLUGIN-COMPILER:REQUIRED-SKILLS -->

## How does one idea become a durable knowledge building block?
...
Ask only when a missing choice would materially change the knowledge captured or
its destination. Otherwise make reversible presentation choices and continue.
Return Markdown in the response unless the user requested file changes.
...
1. **Clarify:** use the naming, removal, and forward-motion tests to expose
   unclear, incomplete, or mixed ideas.
2. **Identify:** classify the building block, then use completeness and
   independent reuse to decide which context belongs with it.
...
every relationship matters. When no collection is available, use
`Suggested connections`, keep targets as plain titles, and do not imply that
they are real files.
...
what changed, where, how it was checked, and what remains unresolved.
