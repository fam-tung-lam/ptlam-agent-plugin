...
async function run(): Promise<void> {
  ...
  if (version === undefined || version.length === 0) {
    throw new Error("RELEASE_VERSION must be a non-empty string.");
  }
  if (outputPath === undefined || outputPath.length === 0) {
    throw new Error("RELEASE_NOTES_PATH must be a non-empty string.");
  }
  await writeFile(
    outputPath,
    extractReleaseNotes(await readFile("CHANGELOG.md", "utf8"), version),
    "utf8",
  );
}
