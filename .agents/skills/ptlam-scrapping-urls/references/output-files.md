...
`docs.example.com-api-auth.md`.

## Resolve collisions

When two different URLs produce the same filename, append the first eight
hexadecimal characters of that URL's SHA-256 digest before `.md`.

Keep the mapping stable across runs, so a later cache check addresses the same
file.
...
Finish when every valid URL maps to one unique path inside the output
directory, and is either `CACHED` or queued exactly once.
