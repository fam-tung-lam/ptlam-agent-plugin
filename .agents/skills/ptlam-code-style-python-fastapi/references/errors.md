...
Keep each feature's domain exception types under `models/failures/` and export
...
controllers and dependency providers for transport-local failures; do not make a
use case, repository, or model import FastAPI to report a status.
