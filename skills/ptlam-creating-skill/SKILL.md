...
description:
  Create, review, or refactor one agent skill so a human maintainer can read it
  once and change it later. Use when turning a workflow or reference set into a
  new skill, revising an existing SKILL.md, splitting a skill that grew too
  broad, or auditing a package without editing it. Use as the foundation for
...
**Reason:** Owns every Mermaid diagram a skill package carries, from type choice to syntax check.

**Instructions:** Read ptlam-mermaiding before judging an existing diagram or deciding
whether a new diagram is warranted.
Apply its authoring workflow only when the chosen form is a diagram.
Let it own the visual question, diagram type, notation, layout, and
syntax check for that diagram.
