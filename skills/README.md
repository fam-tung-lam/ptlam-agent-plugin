...
flowchart TB
    ...
    SkillNode4 --> SkillNode25
    SkillNode4 --> SkillNode28
    ...
    SkillNode18 --> SkillNode27
    SkillNode19 --> SkillNode26
    SkillNode19 --> SkillNode27
    SkillNode19 --> SkillNode23
    SkillNode20 --> SkillNode26
    SkillNode20 --> SkillNode27
    SkillNode21 --> SkillNode27
    SkillNode22 --> SkillNode26
    SkillNode22 --> SkillNode27
    SkillNode23 --> SkillNode27
    SkillNode23 --> SkillNode22
    SkillNode28 --> SkillNode26
    SkillNode29 --> SkillNode23
    SkillNode31 --> SkillNode2
    SkillNode31 --> SkillNode29
    SkillNode32 --> SkillNode16
    ...
    SkillNode34 --> SkillNode7
    SkillNode35 --> SkillNode8
    SkillNode36 --> SkillNode9
...
| `ptlam-creating-adr`                       | Engineering          | Decide whether a crystallized architectural choice warrants a durable architecture decision record and write the qualifying ADR. Use when a confirmed decision splits a component, runtime, or data store, publishes a surface such as an API, SDK, CLI, schema, or file format, moves authoritative state, commits to a platform, or carries material rejected alternatives. Do not use for local implementation choices with cheap reversal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | public     | Active | —           |
| `ptlam-architecturing`                     | Engineering          | Answer one system-level architecture question, for any kind of system from backend and full-stack to web frontend, mobile, SDK, CLI, data pipeline, desktop, embedded, or internal platform, with a suitability judgment that frames the solution space, positions the options, sizes the recommendation for the next order of magnitude, and names its trade-offs, assumptions, and redesign trigger. Use when choosing component, runtime, data-store, deployment, or integration structure; when designing a published surface such as an API, SDK, CLI, schema, or file format; when deciding where authoritative state lives; when a platform limit shapes the design; when a design debate is stuck on technology names; or when judging an existing architecture or proposal against its needs. Compose it from any skill that meets a structure expensive to reverse. Do not use for code structure one team changes in one release, business vocabulary, diagnosing one failure, or writing the ADR alone. | public     | Active | —           |
