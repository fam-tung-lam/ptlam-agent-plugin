...
This reference owns the dimensions behind common architecture debates, the
frame-then-position rule, and the semantics an architecture sketch must carry.

## Frame first, then position

A debate between two technology names has no shared map. Two people can look at
one cylinder and argue circle against rectangle. Build the map before choosing a
path on it:
...
4. Only then position the current state, each option, and the target.

The frame is not up for debate once named; the position is. Disagreement inside
a shared frame converges. Disagreement across two private frames never does.

## Dimensions behind common debates

The worked example: "monolith or microservices" hides two dimensions,
design-time modularity and runtime deployment granularity. Their quadrant holds
four shapes, not two, and names the modular monolith: modular design, one
deployment. The rows below are candidate dimensions for other debates, not
answers; the position is still argued on the frame.
...
| Specific or generic solution                      | Problems solved today            | Problems hypothesized              |
...
| Offline-first or thin client                      | Where authoritative state lives  | Tolerance for staleness            |
...
## Give the sketch real semantics

A picture cannot be fuzzy: two boxes either share a line or they do not. Decide
what every mark means before drawing it, and keep it constant across the sketch.

| Mark                    | Must mean one of                                           |
| ----------------------- | ---------------------------------------------------------- |
| Arrow                   | Data flow, control flow, or dependency; synchronous or not |
| Stacked boxes           | Multiple instances, not exactly the drawn count            |
...
information the sketch exists to extract. A sketch that no one can contradict
carries no claim.
...
current state sit on it, and every mark in the sketch has one declared meaning.
