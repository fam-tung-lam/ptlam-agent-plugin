...
## Names are what people actually read
...
`common`, `shared`, `utils`, and `misc` name a location, not a responsibility,
so unrelated code piles up there until everything depends on it. When a project
keeps one shared area anyway, give it an entry rule (a file arrives only once a
second consumer exists) and split it out under a real name as soon as one
appears.
...
turns every use site into a puzzle.
...
When no name fits, the thing usually does two jobs or matches no real concept.
Split it, or rename the concept it belongs to. A vaguer word hides the design
problem instead of solving it.

## Finish

Finish when every name you added states its role in the domain's words, no name
carries an empty suffix, and every boolean reads as a positive assertion.
