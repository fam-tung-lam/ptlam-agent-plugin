...
hierarchies, such as a bare `String` or `int`, but does not decide the split for
you.
...
| A number is outside a valid range   | `RangeError.range(i, 0, items.length - 1)`    |
...
message says what was wrong without the caller reading source.

## Catch a named type, not everything

Write `on <Type> catch (e, stackTrace)`. `avoid_catches_without_on_clauses`
reports a bare `catch` whose body handles the failure and allows one that only
rethrows. Enable `avoid_catching_errors` too: catching an `Error` subtype hides
a bug behind a recovery path written for a condition.

Convert at the boundary that owns the dependency: catch the library's type
there, and throw or return a type your own callers know. Above that boundary
nothing names the library's exception class.

## Keep the stack trace

`rethrow` keeps both the value and the original trace; `throw e` inside a catch
block restarts the trace at the catch and loses where the failure began.
