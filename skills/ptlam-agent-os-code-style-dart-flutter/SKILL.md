...
description:
  Write, review, and fix Flutter code in the PTLam Agent OS src/app monorepo
  against its app, feature, and package kinds and their dependency direction,
  the ptlam_agent_os_lints rule set and Melos commands, feature registration and
  routing across packages, per-feature localization ownership, the project's
  ...
  wiring a feature into the app router or get_it, adding translations, choosing
  a test level, or fixing a melos or workspace resolution failure. Compose this
  skill from any PTLam Agent OS workflow that changes Dart files. Do not use for
  the four feature layers, widgets, models, or Dart mechanics on their own, or
...
**Reason:** Provides the four-layer feature structure, BLoC and widget rules, models, networking, storage, localization, logging, documentation, and Flutter test mechanics this monorepo's conventions build on.

**Instructions:** Read and apply ptlam-code-style-dart-flutter first; it loads
ptlam-code-style-dart and ptlam-code-style as its own foundations.
Let it own the four feature layers, state holders, widgets, models,
networking, storage, Slang, AppLogger, dartdoc, and Flutter test
mechanics.
Use this skill only for the src/app monorepo: package kinds and
dependency direction, the lints package and Melos commands, feature
registration and routing across packages, localization ownership,
the project's test-level order, and charter rules.
Where this skill names a monorepo mechanic that replaces the
single-package layout, route placement, or test-level order, this
skill wins. Everywhere else it may be stricter, never looser.

Read [ptlam-code-style-dart-flutter](skills/ptlam-code-style-dart-flutter/SKILL.md).
