...
Prefer the dependency's native async API. When none exists and the work is safe
to move to a thread, isolate it with `asyncio.to_thread` or the repository's
approved equivalent. Move CPU-bound work to the project's process or job
boundary rather than occupying the event loop or its I/O thread pool.
...
- Do not create an untracked background task inside a request or library call.
  Give long-lived work a supervisor with an explicit shutdown path.
- Put timeouts at external boundaries and distinguish a timeout from caller
  cancellation.
- Let cancellation propagate after cleanup. Do not turn cancellation into an
  ordinary success or retry.
- Do not share mutable clients, sessions, or buffers across concurrent tasks
  unless their contract says they are safe.
