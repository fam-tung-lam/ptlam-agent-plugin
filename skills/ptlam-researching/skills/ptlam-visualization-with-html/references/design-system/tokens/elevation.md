...
The scaffold owns the exact baseline values. Keep ordinary nested panels on flat
surface colors. Use the first elevation role for an interactive element that
must rise above its surface and the second only for an overlay or focused
learning stage.
...
Source snapshot: Material 3 elevation-token guidance, captured with Firecrawl on
2026-08-07.
